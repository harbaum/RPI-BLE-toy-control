...
if [ "$1" != "" ]; then
    ...
    echo "Kein Gerät angegeben, suche nach fischertechnik-BT-Control-Receiver ..."
	...
	do
		...
		if [ "$FTC" == "" ]; then
		    ...
		    if [ "$OID" == "10:45:F8" ] &&
		       ( [ "$NAME" == "NAME BT Control Receiver" ] ||
			 [ "$NAME" ==      "BT Control Receiver" ] ); then
			
			echo "fischertechnik-BT-Control-Receiver erkannt: $MAC"
			FTC=$MAC
...
CH_SRV=`gatttool -b $FTC --primary -u 2e582b3a-c5c5-11e6-9d9d-cec0c932ce01`
if [ "$CH_SRV" != "" ]; then
    ...
    CH_CHAR=`gatttool -b $FTC --char-desc -s 0x$SH -e 0x$EH | grep 2e582de2-c5c5-11e6-9d9d-cec0c932ce01`
